...
extension AttendanceStatus {
    /// Firestore の `status` 文字列からステータスを生成する。
    /// 仮データは日本語（"出席" / "遅刻" など）だが、
    /// ER 図の英語表記（present / absent / late / early_leave / mid_absence / excused）も許容する。
    init?(firestoreValue raw: String) {
        let normalized = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalized.isEmpty else { return nil }

        if let direct = AttendanceStatus(rawValue: normalized) {
            self = direct
            return
        }
        ...
        case "late":
            self = .tardiness
        case "early_leave", "earlyleave", "early_departure":
            self = .earlyDeparture
        case "mid_absence", "midabsence":
            self = .earlyDeparture
        case "excused", "official_absence":
            self = .officialAbsence
        case "bereavement":
            self = .bereavement
        default:
            return nil
